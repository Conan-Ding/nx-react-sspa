...
export async function normalizeOptions(tree: Tree, schema: ReactSingleSpaNxGeneratorSchema): Promise<NormalizedSchema> {
    const options = await determineProjectNameAndRootOptions(tree, {
        name: schema.name,
        projectType: 'application',
        callingGenerator: 'react-sspa',
        directory: schema.directory,
        projectNameAndRootFormat: schema.projectNameAndRootFormat || 'as-provided',
    });
